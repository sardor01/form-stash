...
export function App() {
  ...
  useEffect(() => {
    const reset = setTimeout(setError, 4000, null)
  ...
  async function handleEdit(preset: Preset) {
    ...
    if (!newLabel || newLabel === preset.label)
      return
    await upsertPreset({
      ...preset,
      label: newLabel.trim(),
  ...
  async function handleDeleteForm(form: FormDef) {
    ...
    if (!choice)
      return
    if (choice === 'delete')
      await deleteForm(form.id, 'delete-steps')
    else if (choice === 'detach')
      await deleteForm(form.id, 'detach-steps')
  }
  ...
  async function moveStep(preset: Preset, delta: -1 | 1) {
    ...
    if (nextIdx < 0 || nextIdx >= stepsForForm.length)
      return
    ...
    <div className="flex flex-col gap-2 border-b border-slate-200 pb-3">
      ...
      {store.loaded && store.presets.length === 0 && (
        <div className="text-xs text-slate-500">
          No presets yet — fill a form, then click
          {' '}
          <strong>Capture this page</strong>
          {' '}
          or
