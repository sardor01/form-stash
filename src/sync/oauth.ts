...
export async function signInWithGoogle(
  ...
  if (!googleClientId) throw new Error('googleClientId is required');
  ...
  if (!redirectedTo) throw new Error('OAuth flow returned no redirect URL');
  ...
  if (claims.nonce !== nonce) throw new Error('OAuth nonce mismatch');
