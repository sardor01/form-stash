...
import type { StoredBlob, SyncBucket, SyncSession } from './types';
...
async function runSyncInner(): Promise<SyncResult> {
  ...
  if (!config.enabled) return idle('sync disabled');
...
async function ensureValidSession(
  ...
  if (session?.idToken && session.idTokenExp - 60 > now) return session;
  if (!clientId)
...
function sameIds(a: Identified[], b: Identified[]): boolean {
  ...
  for (const r of b) {
    if (am.get(r.id) !== r.updatedAt) return false;
  }
  return true;
}
...
export async function setPassphrase(passphrase: string): Promise<void> {
  ...
  if (!session?.idToken) throw new Error('sign in with Google first');
  const client = new WorkerClient(config.workerUrl, session.idToken);
